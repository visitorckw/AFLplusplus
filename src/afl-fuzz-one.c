...
u8 fuzz_one_original(afl_state_t *afl) {
  ...
  if (likely(afl->not_on_tty)) {
    ...
    ACTF(
        ...
        "exec_us=%llu, hits=%u, map=%u, interesting=%u/%u, ascii=%u, "
        "run_time=%s)...",
        ...
        afl->queue_cur->bitmap_size, afl->queue_cur->loop_cnt,
        afl->queue_cur->func_cnt, afl->queue_cur->is_ascii, time_tmp);
    fflush(stdout);
